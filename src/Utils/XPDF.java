...
public class XPDF {
    ...
    public void createFilePDF(HDBanLe hDBanLe, List<CTHDBanLe> sp) throws FileNotFoundException, IOException {
        ...
        this.document.setFont(fontPDF("/icon/Lora-VariableFont_wght.ttf"));
        ...
        for (int i = 0; i < sp.size(); i++) {
            for (int j = 0; j < 4; j++) {
                if (j == 0) {
                    ...
                    cell.setPadding(0);
                    cell.setBorder(Border.NO_BORDER);
                    cell.add(new Paragraph(sp.get(i).getMaSP()));
                    if (i == sp.size() - 1) {
    ...
    public PdfFont fontPDF(String str) throws IOException {
        return PdfFontFactory.createFont(getClass().getResource(str).getPath().substring(1), PdfFontFactory.EmbeddingStrategy.PREFER_EMBEDDED);
    }
